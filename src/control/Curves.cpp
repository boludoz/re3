...
inline float DistToMathematicalLine2D(float lineX, float lineY, float dirX, float dirY, float pointX, float pointY)
{
...
// func: sa 0x43C610
float CCurves::DistForLineToCrossOtherLine(float LineBaseX, float LineBaseY, float LineDirX, float LineDirY,
...
// func: sa 0x43C660
float CCurves::CalcSpeedVariationInBend(const CVector& startCoors, const CVector& endCoors,
...
// func: sa 0x43C710
float CCurves::CalcSpeedScaleFactor(const CVector& startCoors, const CVector& endCoors,
	float StartDirX, float StartDirY, float EndDirX, float EndDirY)
{
...
float CCurves::CalcCorrectedDist(float Current, float Total, float SpeedVariation, float* pInterPol)
{
	...
	*pInterPol = 0.5f - (Cos(PI * (Current / Total)) * 0.5f);
	...
	float CorrectedDist = AverageSpeed * (Total * (1.0f / TWOPI)) * SpeedVariation +
		((1.0f - (SpeedVariation + SpeedVariation) + 1.0f) * 0.5f) * Current;
...
void CCurves::CalcCurvePoint(const CVector& startCoors, const CVector& endCoors, const CVector& startDir,
	const CVector& endDir, float Time, int32 TraverselTimeInMillis, CVector& resultCoor, CVector& resultSpeed)
{
